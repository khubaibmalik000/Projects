...
| Project | Description |
|---|---|
| [aiops-incident-intelligence-platform](aiops-incident-intelligence-platform/) | AIOps engine: EWMA/z-score + Isolation Forest anomaly detection, Drain-inspired log template mining, dependency-graph alert correlation with root-cause ranking, and a dry-run-by-default remediation rule engine. FastAPI server (SQLAlchemy/Postgres) plus a standalone node agent for real metric/log collection, Docker Compose, Kubernetes manifests, 53-test suite — live-verified end-to-end across two simulated nodes. |
...
| [postgres-master-slave-failover-vps](postgres-master-slave-failover-vps/) | Live-verified edition of the above against two real Ubuntu VPS hosts — found and fixed 5 real Debian/Ubuntu packaging gaps `pg_rewind` hits that the Docker-derived runbook doesn't cover. |
